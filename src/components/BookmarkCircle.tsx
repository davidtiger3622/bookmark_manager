  ...
  return (
    <div className="group flex w-20 flex-col items-center gap-2">
      <div className="relative">
        <button onClick={() => window.open(bookmark.url, "_blank", "noopener,noreferrer")} className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--bg-card)] shadow-md transition hover:scale-105">
      ...
      <span className="w-full truncate rounded-full bg-[var(--bg-card)]/85 px-2 py-0.5 text-center text-xs font-bold text-[var(--text)] shadow-sm backdrop-blur-sm">{bookmark.name}</span>
    </div>
  );
}
