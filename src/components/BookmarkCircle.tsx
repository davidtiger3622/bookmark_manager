  ...
  return (
    <div className="group flex w-20 flex-col items-center gap-2">
      <div className="relative">
        ...
        <button onClick={() => onToggleFavorite(bookmark.id)} aria-label="Toggle favorite" className={`absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-[var(--bg-card)] text-xs opacity-0 shadow transition group-hover:opacity-100 ${bookmark.favorite ? "opacity-100 text-red-500" : "text-white"}`}>
          &#9829;
        </button>
      </div>
      <span className="w-full truncate text-center text-xs font-bold text-white">{bookmark.name}</span>
    </div>
  );
}
